...
export function PreparationChecklist({ preparations, completedIds, allCompleted, onToggle, onConfirm, onSkip, disabled = false }: PreparationChecklistProps) {
  ...
  return (
    <section aria-labelledby="preparation-checklist-heading" className="space-y-5 rounded-2xl border bg-card p-5">
      ...
      <ul className="space-y-3">
        {sortRecipePreparations(preparations).map((preparation) => (
          <li className="flex items-start gap-3 rounded-xl border p-4" key={preparation.id}>
            <input aria-label={`完成：${preparation.instruction}`} checked={completed.has(preparation.id)} className="mt-1 size-5 accent-primary" disabled={disabled} onChange={() => onToggle(preparation.id)} type="checkbox" />
      ...
      <div className="flex flex-wrap gap-3">
        <Button className="min-h-11" disabled={disabled || !allCompleted} onClick={onConfirm} type="button">准备完成，开始烹饪</Button>
        <Button className="min-h-11" disabled={disabled} onClick={onSkip} type="button" variant="outline">仍然开始烹饪</Button>
      </div>
    </section>
  );
}
