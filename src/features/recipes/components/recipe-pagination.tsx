...
function buildPageHref(query: RecipeListQuery, page: number) {
...
export function RecipePagination({ query, totalCount }: { query: RecipeListQuery; totalCount: number }) {
  const totalPages = Math.ceil(totalCount / PAGE_SIZE);
  if (totalPages <= 1) return null;

  return (
    <nav aria-label="菜谱分页" className="flex items-center justify-center gap-3 pt-2">
      {query.page > 1 && <Link className="rounded-lg border px-3 py-2 text-sm" href={buildPageHref(query, query.page - 1)}>上一页</Link>}
      <span className="text-sm text-muted-foreground">第 {query.page} / {totalPages} 页</span>
      {query.page < totalPages && <Link className="rounded-lg border px-3 py-2 text-sm" href={buildPageHref(query, query.page + 1)}>下一页</Link>}
    </nav>
  );
}
