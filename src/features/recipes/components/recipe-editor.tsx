  ...
  return (
    <form className="space-y-8" onSubmit={handleSubmit(onSubmit, () => setServerMessage("请检查菜谱内容后再保存"))}>
      ...
      <section className="space-y-4 rounded-2xl border bg-card p-5">
        ...
        <div className="space-y-4">{ingredientFields.fields.map((field, index) => <div className="grid gap-3 rounded-xl border p-4 md:grid-cols-[1.5fr_0.7fr_0.7fr_0.7fr_0.8fr_auto]" key={field.fieldKey}><div className="space-y-1"><Label htmlFor={`ingredient-${index}`}>食材名称</Label><Input id={`ingredient-${index}`} {...register(`ingredients.${index}.name`, { required: "请先填写食材名称" })} />{errors.ingredients?.[index]?.name && <p className="text-sm text-destructive">{errors.ingredients[index]?.name?.message}</p>}</div><div className="space-y-1"><Label htmlFor={`quantity-${index}`}>数量</Label><Input id={`quantity-${index}`} min={0} step={0.001} type="number" {...register(`ingredients.${index}.quantity`, { valueAsNumber: true })} /></div><div className="space-y-1"><Label htmlFor={`unit-${index}`}>单位</Label><Input id={`unit-${index}`} placeholder="克、个、勺" {...register(`ingredients.${index}.unit`)} /></div><div className="space-y-1"><Label htmlFor={`quantity-text-${index}`}>文字用量</Label><Input id={`quantity-text-${index}`} {...register(`ingredients.${index}.quantityText`)} /></div><div className="space-y-1"><Label htmlFor={`group-${index}`}>分组</Label><select className="h-9 w-full rounded-md border bg-background px-2 text-sm" id={`group-${index}`} {...register(`ingredients.${index}.groupType`)}><option value="main">主料</option><option value="seasoning">调料</option><option value="other">其他</option></select></div><div className="flex items-end gap-1"><Button aria-label={`上移食材 ${index + 1}`} className="px-2" disabled={index === 0} onClick={() => moveIngredient(index, -1)} type="button" variant="ghost">↑</Button><Button aria-label={`下移食材 ${index + 1}`} className="px-2" disabled={index === ingredientFields.fields.length - 1} onClick={() => moveIngredient(index, 1)} type="button" variant="ghost">↓</Button><Button aria-label={`移除食材 ${index + 1}`} className="px-2" onClick={() => removeIngredient(index)} type="button" variant="ghost">移除</Button></div></div>)}</div>
      </section>

      <section className="space-y-4 rounded-2xl border bg-card p-5">
        <div className="flex items-center justify-between gap-4"><div><h2 className="text-xl font-semibold">步骤</h2><p className="text-sm text-muted-foreground">每一步可以关联当前要用的食材并设置火候和计时。</p></div><Button onClick={() => stepFields.append({ stepId: crypto.randomUUID(), instruction: "", imagePath: null, timerSeconds: null, heatLevel: null, sortOrder: stepFields.fields.length, ingredientLinks: [] })} type="button" variant="outline">添加步骤</Button></div>
