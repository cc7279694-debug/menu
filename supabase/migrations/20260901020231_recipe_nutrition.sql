create table public.recipe_nutrition (
  ...
  constraint recipe_nutrition_user_fk
  ...
  constraint recipe_nutrition_calories_range
    check (calories_kcal is null or (calories_kcal >= 0 and calories_kcal <= 100000)),
  constraint recipe_nutrition_protein_range
    check (protein_grams is null or (protein_grams >= 0 and protein_grams <= 10000)),
  constraint recipe_nutrition_fat_range
    check (fat_grams is null or (fat_grams >= 0 and fat_grams <= 10000)),
  constraint recipe_nutrition_carbs_range
    check (carbs_grams is null or (carbs_grams >= 0 and carbs_grams <= 10000)),
  constraint recipe_nutrition_has_value
    check (
...
create index recipe_nutrition_user_recipe_idx
  on public.recipe_nutrition (user_id, recipe_id);
...
alter table public.recipe_nutrition force row level security;

create policy recipe_nutrition_select on public.recipe_nutrition for select to authenticated
using ((select auth.uid()) = user_id);
create policy recipe_nutrition_insert on public.recipe_nutrition for insert to authenticated
with check ((select auth.uid()) = user_id);
create policy recipe_nutrition_update on public.recipe_nutrition for update to authenticated
using ((select auth.uid()) = user_id)
with check ((select auth.uid()) = user_id);
create policy recipe_nutrition_delete on public.recipe_nutrition for delete to authenticated
using ((select auth.uid()) = user_id);

revoke all on table public.recipe_nutrition from public, anon, authenticated;
grant select, insert, update, delete on table public.recipe_nutrition to authenticated;
...
declare
  ...
  v_nutrition jsonb := p_payload->'nutrition';
  v_calories_kcal numeric;
  v_protein_grams numeric;
  v_fat_grams numeric;
  v_carbs_grams numeric;
...
begin
  ...
  if v_nutrition is null or pg_catalog.jsonb_typeof(v_nutrition) = 'null' then
    delete from public.recipe_nutrition
    where user_id = v_user_id and recipe_id = v_recipe_id;
  else
    v_calories_kcal := nullif(v_nutrition->>'caloriesKcal', '')::numeric;
    v_protein_grams := nullif(v_nutrition->>'proteinGrams', '')::numeric;
    v_fat_grams := nullif(v_nutrition->>'fatGrams', '')::numeric;
    v_carbs_grams := nullif(v_nutrition->>'carbsGrams', '')::numeric;
    v_is_estimated := coalesce((v_nutrition->>'isEstimated')::boolean, true);

    if v_calories_kcal is null
      and v_protein_grams is null
      and v_fat_grams is null
      and v_carbs_grams is null then
      delete from public.recipe_nutrition
      where user_id = v_user_id and recipe_id = v_recipe_id;
    else
      insert into public.recipe_nutrition (
        user_id, recipe_id, calories_kcal, protein_grams, fat_grams, carbs_grams, is_estimated
      ) values (
        v_user_id, v_recipe_id, v_calories_kcal, v_protein_grams,
        v_fat_grams, v_carbs_grams, v_is_estimated
      )
...
as $$
  ...
  left join public.recipe_nutrition n on n.user_id = p.user_id and n.recipe_id = p.id
